...
Just a reminder that the best way to do this is to copy and paste the entire block of code above into your '.R' file, highlight it, and press run.
...
Take a second to read through the code. Even though you may not understand all of it, can you find the part where the charge is calculated using equations 4 & 5? Note that doing math in R is generally the same as typing it in on a calculator. You can use all of the normal operators and symbols like `+ - / * ^ ()`.
...
*If you aren't exactly sure what a function is, try reading [this definition](https://www.cs.utah.edu/~germain/PPS/Topics/functions.html). The short answer is that a function is a command you give the computer that carries out some specfic task or calculation.*
