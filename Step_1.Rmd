...
$$
From these equations, we can calculate the charge on any one residue (or terminus) if we know its pKa, whether it is acidic or basic, and the pH. Then by counting the number of each potentially ionizable groups in a peptide, we can add up their respective charges to arrive at a net charge for the peptide for any given pH.

### Code

The function below: `pep_charge()`, takes two arguments: `peptide` which is a variable that corresponds to a peptide written out in one letter code, and `pH` which is a number between 0 and 14 designating the pH of interest. You can copy and paste this entire block of code into the code editor, highlight it, and click run to have the function availible to you. (You will need to add the `stringr` package if you don't already have it installed. See [this link](http://derekogle.com/IFAR/supplements/installations/InstallPackagesRStudio.html) for installing R packages.)
...
### Exercise (1)
The exercise for step one is simple. Simply run the function with an example peptide: DIAK. 
...
After ensuring that you have loaded the `pep_charge()` function, try it out in the terminal. We will set `peptide = "DIAK"` and `pH = 7.0` first. Type or copy and paste the code below into your terminal and press enter.
```{r}
pep_charge(peptide = "DIAK", pH = 7.0)
```
The first printout is the dataframe that we used to create each charge. Can you tell what each column is for?  

The second printout is the net charge that we calculated. It is negative, but very close to 0, indicating that this peptide is mostly neutral at pH 7.0, even though there are 4 charged groups. This is because the positive charges of the N-terminus and the Lysine cancel out the negative chares of the Aspartate and the C-terminus. Lets try the same thing at pH 3.0.
```{r}
pep_charge(peptide = "DIAK", pH = 3.0)
```
At this lower pH, the charge of the peptide is more than 1.5 and less than 2. Why is this? Both of the basic residues are essentially fully charged at this pH (see the `net_charge` column), but only about 10% and 20% of the Asp and C-term are negatively charged at pH 3.0. When these charges are added up, we get a charge of about +1.7 for the net charge of the peptide.

Take screenshots of your terminal showing the dataframe and net charge for DIAK at pH 7 and pH 3, and paste this into your assignment. Write a sentence predicting what the charge will be at pH 12, and feel free to run the function at that pH to check your answer.