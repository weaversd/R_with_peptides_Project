...
for (pep in peptides) {
  print(pep)
  print(pep_length(peptide = pep))
}
```
If you ran the peptides list commmand earlier, and the new block of code, you should see an output that printed out each peptide followed by the number of amino acids present. Here are what the first three printouts should look like: 
```{r, echo=FALSE}
...
```
Lets try it on one of our favorite peptides, DIAK. You should run this on your own terminal as well.
```{r}
m_over_z("DIAK", pH = 7)
```
...
pep_mass("DIAK")
```
So the mass looks reasonable, but the charge is very close to zero (we actually knew this already from exercise 1). So when we are dividing mass / charge, we are dividing by a number that is essentially zero, causing the m/z value to balloon to an unrealistic value.  

If you think about MALDI however, we don't actually know what the pH is, and so assuming it to be 7 is probably not accurate. In addition, based on your extensive knowledge of MALDI for peptides, you remember that almost all peptides end up with a +1 charge (due to one proton) when they are ionized with this technique. How could you modify the `m_over_z()` function to make it applicable to MALDI? The important aspects are: 

* The charge will always be +1
* The mass will be the mass of the peptide *plust the mass of a proton* which is approximately 1.008 Da.  
